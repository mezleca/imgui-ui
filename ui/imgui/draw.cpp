...
namespace ui {
    ...
    // maximum sagitta error, matching imgui adaptive circle tessellation model.
    static constexpr float ARC_MAX_ERROR = 0.25F;
    ...
    static void
    walk_selected_segments(const BorderPath& path, uint8_t border, OnSegment&& on_segment, OnRunBreak&& on_run_break) {
    ...
    static void stroke_solid_path(const BorderPath& path, uint8_t border, ImU32 color, float thickness) {
    ...
    static void stroke_dashed_path(const BorderPath& path, uint8_t border, ImU32 color, float thickness) {
        ImDrawList& draw_list = *ImGui::GetWindowDrawList();
    ...
    static void stroke_dotted_path(const BorderPath& path, uint8_t border, ImU32 color, float thickness) {
        ImDrawList& draw_list = *ImGui::GetWindowDrawList();
    ...
    void draw_frame(Rect rect, const Style& style, ImColor background) {
        ...
        if (style.border() == BORDER_ALL && style.border_style() == BorderStyle::Solid) {
            Style opaque_style = style;
            opaque_style.border_color(border);
            draw_full_frame(rect, opaque_style, background);
            return;
        }
    ...
    void draw_border_path(const BorderPath& path, uint8_t border, ImColor color, float thickness, BorderStyle style) {
        ...
        switch (style) {
            case BorderStyle::Solid:
                stroke_solid_path(path, border, draw_color, thickness);
                return;
            case BorderStyle::Dashed:
                stroke_dashed_path(path, border, draw_color, thickness);
                return;
            case BorderStyle::Dotted:
                stroke_dotted_path(path, border, draw_color, thickness);
                return;
        }
    }
    ...
    void draw_border(Rect rect, const Style& style, ImColor color) {
        if (style.border() == BORDER_ALL && style.border_style() == BorderStyle::Solid) {
            ImGui::GetWindowDrawList()->AddRect(rect.min, rect.max, color, style.border_radius(), style.border_thickness());
            return;
        }

        draw_border_path(
            border_path(rect, style.border_radius()), style.border(), color, style.border_thickness(), style.border_style()
        );
    }
} // namespace ui
