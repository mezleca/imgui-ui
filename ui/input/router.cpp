...
namespace ui {
    ...
    bool InputRouter::dispatch(UiEvent& event) {
        ...
        if (event.type == EventType::PointerDown) {
            if (const std::optional<std::size_t> button_index = pointer_button_index(event.button); button_index.has_value()) {
    ...
    std::optional<InputLayer> InputRouter::highest_blocking_layer(EventType type) const {
        ...
        for (std::size_t index = m_policies.size(); index-- > 0;) {
            ...
            if ((pointer_event && policy != InputPolicy::PassThrough) || (!pointer_event && policy == InputPolicy::BlockAll)) {
