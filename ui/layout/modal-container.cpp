...
ModalContainer::ModalContainer(UI& ui)
    ...
    set_input_blocker({.on_event = [this](UiEvent& event) { handle_event(event); }});
    configure_all_styles([](Style& style) { style.background_color(ImColor{0.0F, 0.0F, 0.0F, 0.42F}); });

    _on_event = [this](UiEvent& event) { handle_event(event); };
}

void ModalContainer::handle_event(UiEvent& event) {
    if (!has_open_modal()) {
        return;
    }
    ...
    const bool pressed_escape = event.type == EventType::Cancel || (event.type == EventType::KeyDown && event.key == Key::Escape);
